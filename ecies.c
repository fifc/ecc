...
secure_t * ecies_encrypt(char *pubkey, unsigned char *data, size_t length) {
	...
	iv[EVP_MAX_IV_LENGTH], block[EVP_MAX_BLOCK_LENGTH];
        ...
        if (!(user = ecies_key_create_public_hex(pubkey))) {
...
unsigned char * ecies_decrypt(char *privkey, secure_t *cryptex, size_t *length) {
        size_t key_length;
        int output_length;
        EC_KEY *user, *ephemeral;
#ifdef __HMAC__
	unsigned char md[EVP_MAX_MD_SIZE];
        ...
        // Convert the user's public key from hex into a full EC_KEY structure.
        else if (!(user = ecies_key_create_private_hex(privkey))) {
